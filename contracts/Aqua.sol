...
contract aqarchain is Ownable{
    ...
    function seedusdt(string calldata _first,string calldata _last,string calldata _country,string calldata _id, uint256 _amount)  external returns (string memory aqarid){
        require(_amount>=100000000000000000000 && _amount<=25000000000000000000000,"Enter some amount");
        require(amountmaptouserseed[_id]<=100000000000000000000000,"Enough token purchased from seed round");
      ...
      function seedbnb(string calldata _first,string calldata _last,string calldata _country,string calldata _id) external payable {
        ...
        require(
            msg.value.mul(getBnbRate()).div(1e18) >= 100000000000000000000 &&   msg.value.mul(getBnbRate()).div(1e18)<= 25000000000000000000000,
            "the input bnb amount should be greater than hundred and less than twentyfivethousand"
        );
        require(amountmaptouserseed[_id]<=100000000000000000000000,"Enough token purchased from seed round");
        require(seedamount<=7000000000000000000000000,"seed round token sale completed");
        require(seedrun = true,"seed round is not started or over");
     ...
     function privateusdt(string calldata _first,string calldata _last,string calldata _country,string calldata _id, uint256 _amount)  external returns (string memory aqarid){
        require(_amount>=1000000000000000000000 && _amount<=100000000000000000000000,"Enter some amount");
        require(amountmaptouserprivate[_id]<=285700000000000000000000,"Enough token purchased from private round");
      ...
      function privatebnb(string calldata _first,string calldata _last,string calldata _country,string calldata _id) external payable {
        // user enter amount of ether which is then transfered into the smart contract and tokens to be given is saved in the mapping
       require(
           msg.value.mul(getBnbRate()).div(1e18) >= 1000000000000000000000 && msg.value.mul(getBnbRate()).div(1e18)<= 100000000000000000000000,
            "the input bnb amount should be greater than hundred and less than twentyfivethousand"
        );
        require(privaterun=true,"Private sale haven't started yet");
         require(amountmaptouserprivate[_id]<=285700000000000000000000,"Enough token purchased from private round");
     ...
     function publicusdt(string calldata _first,string calldata _last,string calldata _country,string calldata _id, uint256 _amount)  external returns (string memory aqarid){
         require(_amount>=100000000000000000000 && _amount<=5000000000000000000000,"Enter some amount");
         require(amountmaptouserprivate[_id]<=11000000000000000000000,"Enough token purchased from private round");
    ...
    function publicbnb(string calldata _first,string calldata _last,string calldata _country,string calldata _id) external payable {
        // user enter amount of ether which is then transfered into the smart contract and tokens to be given is saved in the mapping
        require(
             msg.value.mul(getBnbRate()).div(1e18) >= 100000000000000000000 &&  msg.value.mul(getBnbRate()).div(1e18) <= 5000000000000000000000,
    ...
    function claim() external {
        ...
        claimamount = amountmaptouserpublic[usermappublic[msg.sender].aqarid].add(amountmaptouserseed[usermapseed[msg.sender].aqarid]).add(amountmaptouserprivate[usermapprivate[msg.sender].aqarid]);
        token.transfer(msg.sender,claimamount);
        amountmaptouserpublic[usermappublic[msg.sender].aqarid]=0;
        amountmaptouserprivate[usermapprivate[msg.sender].aqarid]=0;
        amountmaptouserseed[usermapseed[msg.sender].aqarid]=0;
    }
